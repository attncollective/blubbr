...
export default function useLensTimeline() {
    // wagmi hooks
    ...
    const {
        ...
        setLoadingTimeline: setLoading,
        timeline,
        setTimeline,
        timelineError: error,
        setTimelineError: setError,
    } = useLensContext()

    async function getTimelineByProfileId() {
        setLoading(true)
        getTimeline({
            variables: {
                request: {
            ...
            .then((data) => {
                if (data.data && data.data.timeline && data.data.timeline.items) {
                    ...
                    setError('Timeline: no data fetched!')
                    setLoading(false)
                }
            })
            .catch((err) => {
    ...
    useEffect(() => {
        if (isConnected && isLoggedIn && hasProfile) {
